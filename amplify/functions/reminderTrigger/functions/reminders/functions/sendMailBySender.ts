...
import { markTriggerAsSent } from "../../database/dbOperations";
        ...
        tasks.push(
            send(sendProps)
                .then(async (res) => {
                    ...
                    await Promise.all(
                        triggers.map((trigger) => {
                            ...
                            return markTriggerAsSent(trigger.trigger.id);
                        })
                    );
                    return res;
                })
                .catch((err) => {
                    ...
                    console.log("Failed to send mail", event.type, trigger.trigger.type, trigger.influencer.emailLevel);
...
function groupBySender(triggers: EmailTriggerData[]) {
    ...
    triggers.forEach((trigger) => {
        ...
        const sender = firstName && lastName && email ? `${firstName} ${lastName} <${email}>` : "none";
