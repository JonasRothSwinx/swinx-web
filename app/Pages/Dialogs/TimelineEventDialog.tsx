    ...
    const EventHandlers = {
        ...
        onSubmit: (event: React.FormEvent<HTMLFormElement>) => {
            ...
            if (editing && dates.dates[0]) {
                ...
                const newEvents: TimelineEvent.Event[] = dates.dates
                    .map((date): TimelineEvent.Event | undefined => {
                        if (date === null) return;
                        return {
                            ...timelineEvent,
                            assignment: {
                                id: timelineEvent.assignment.id,
                                influencer: timelineEvent.assignment.influencer,
                ...
                // debugger;
                Promise.all(newEvents.map((x) => x && timelineEvents.create(x))).then((res) =>
                    console.log(newEvents, res),
                );
                newEvents.map((x) => (x.id = randomId()));
        ...
        handleAssignmentChange: (e: SelectChangeEvent) => {
            const value = e.target.value;
        },
        handleRemoveDateClick: (i: number) => {
            ...
            <DialogContent dividers sx={{ "& .MuiFormControl-root": { flexBasis: "100%" } }}>
                ...
                <FormControl sx={{ margin: "5px", flex: 1, minWidth: "200px" }}>
                    ...
                    <Select
                        ...
                        onChange={EventHandlers.handleAssignmentChange}
                        required
                    >
                            ...
                            return (
                                <div key={`dumb${index}`} className={styles.cellActionSplit}>
                                    <DatePicker
                                        ...
                                        name="date"
                                        value={date}
                                        onChange={EventHandlers.handleDateChange(index)}
                                        slotProps={{
                                            textField: {
                                                required: true,
                                            },
